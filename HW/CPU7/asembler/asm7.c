...
int main(int argc, char **argv){
	...
	while(ReadLine(fileIn, buff)){
        ...
        if(strlen(word1)==0) continue;
			...
			int num=atoi(word2);
			if(num>2047) printf("ERROR: Line %d\n", line);

			if(strcmp(word1, "add")==0) 	 writeCode(fileOut, 0x0800|num);
			else if(strcmp(word1, "nor")==0) writeCode(fileOut, 0x1800|num);
			else if(strcmp(word1, "xor")==0) writeCode(fileOut, 0x2800|num);
			else if(strcmp(word1, "lda")==0) writeCode(fileOut, 0x3800|num);
			else if(strcmp(word1, "sta")==0) writeCode(fileOut, 0x4000|num);
			else if(strcmp(word1, "jmp")==0) writeCode(fileOut, 0x8000|num);
			else if(strcmp(word1, "jeq")==0) writeCode(fileOut, 0xC000|num);
			else printf("ERROR: Line %d\n", line);
		}
	}
	fclose(fileIn);
